...
| Solution entry point | `solution/publish.sh` → runs the reference implementation in solution/release-publisher.mjs | |
...
- **Kept (abstract only):** the task type (build-from-spec), the skill set, the
  ...
  reward).
- **Reference solution:** The reference implementation is included separately under 
   solution/. solution/publish.sh is the reference entry point and runs 
   solution/release-publisher.mjs.The reference implementation is not shipped 
   inside environment/publisher/.